...
from semiseg import networks, models
...
def run_experiment(experiment_config: Dict, load_weights: bool, save_weights: bool) -> None:
    ...
    assert dataset_name in ['cityscapes'], 'The dataloader is only implemented for citysapes dataset'
    data_loader = CityscapesLoader(label_percent=labels_percent).get_cityscapes_loader(
        batch_size=training_args['batch_size'],
        num_workers=training_args['num_workers'],
        mode=mode)
    models_module = importlib.import_module('semiseg.models')
    model_class_ = getattr(models_module, experiment_config['model'])

    networks_module = importlib.import_module('semiseg.networks')
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, mode='max', factor=0.1, patience=15, min_lr=1e-10,
                                                              verbose=True)
    additional_identifer = get_additional_identifier(network_args['backbone'], pretrained, dataset_name, labels_percent)

    model = model_class_(network, data_loader, optim, lr_scheduler=lr_scheduler,
                         additional_identifer=additional_identifer)
    ...
    if load_weights:
        ...
        selfsup_model = models.SelfSupervisedModel(byol, additional_identifer=additional_identifer)
        print('loading self-supervised weights from ', selfsup_model.weights_file_name)
        byol_state_dict = torch.load(selfsup_model.weights_file_name)
        backbone_dict = {}
...
def get_additional_identifier(backbone: str, pretrained: bool = False,
                              dataset_name: str = '', labels_percent: str = '100%') -> str:
    """
