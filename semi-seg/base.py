...
logs_dir = dirname / "tensorboard_logs"
...
class Model:
    ...
    def __init__(self, network: torch.nn.Module,
        ...
        self.name = f"{self.__class__.__name__}_{self.network.__class__}"
    ...
    def weight_file_name(self) -> str:
        """
        :return: returns the path where to strore/load the weights of the model
        """
        weights_dir.mkdir(parents=True, exist_ok=True)
        return str(weights_dir / f'{self.name}.h5')

    def train(self, num_epochs: int) -> None:
        """
        1. Set up the model in appropriate training mode(Use this to set up DDP model for distributed training also if required)
        2. Train the model for num_epochs and keep track of the best_model_weights based on self.metric[0] values.
        3. Log the epoch results (loss and other available statistics mentioned in self.metrices) on tensorboard
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if self.lr_scheduler is not None:
                self.lr_scheduler.step(epoch_stats[0])
                ...
                if lr_step / min_lr <= 10:
                    print('Minimum Learning Rate Achieved')
                    break
