...
class CityscapesDataset(Dataset):
    def __init__(self, cityscapes_data: Union[Dataset, List], label_percent: int = 100, transform: Callable = None,
                 ...
                 is_test: bool = False, mode: str = 'supervised'):
        """Custom Cityscapes dataset to apply some transformation on image and mask pair
        ...
        """
        inputs = functools.reduce(lambda a, b: a + b.images, cityscapes_data, []) if type(
            cityscapes_data) == list else cityscapes_data.images
        total = len(inputs)

        n = int(total * label_percent) // 100
        self.imgs_path = input[:n]
        ...
        assert self.mode in ['supervised',
                             'self-supervised'], "Invalid dataset mode. Only 'supervised', " "'self-supervised' is allowed"
    ...
    def get_random_crop(self, image: Image, crop_size: int) -> Image:
        ...
        :return: PIL Image crop of crop_size
        """
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'self-supervised':
            ...
            crop_tensor = torchvision.transforms.Compose([*get_pixelwise_tfms(),
                                                          torchvision.transforms.ToTensor()])(crop)
