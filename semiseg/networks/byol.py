...
class ProjectorHead(nn.Module):
    ...
    def forward(self, x):
        x_pooled = self.avg_pool
        h = x_pooled.view(x_pooled.shape[0], x_pooled.shape[1])
...
class BYOL(BYOL_Head):
    def __int__(self, backbone: nn.Module, target_momentum=0.996):
        super().__int__(backbone, target_momentum)
